...
import { ChevronDown, Plus, X, Check, Clock, Zap, Flame, Calendar, TrendingUp, Volume2, VolumeX, Settings } from 'lucide-react';
...
export default function StudyTracker() {
  ...
  useEffect(() => {
    const savedTasks = localStorage.getItem(`tasks_${currentDate}`);
  ...
  // Save daily history to localStorage
  useEffect(() => {
  ...
  const updateTaskStatus = (id, status) => {
    ...
    if (soundEnabled && status === 'completed') {
      playSound('complete');
    } else if (soundEnabled && status === 'failed') {
      playSound('fail');
    }
  };
  ...
  const saveDay = () => {
    ...
    const existingIndex = dailyHistory.findIndex(d => d.date === currentDate);
    if (existingIndex >= 0) {
    ...
    alert(`Day saved! Discipline Score: ${disciplineScore}%`);
  };
  ...
  const getStreak = () => {
    ...
    for (let i = 0; i < sorted.length; i++) {
      ...
      if (dayDate.toDateString() === expectedDate.toDateString()) {
        if (sorted[i].completed_percentage >= 70) {
          streak++;
        } else {
          break;
        }
  ...
  const getCalendarDays = () => {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      days.push({
        date: i,
        dateStr,
        score: dayData?.disciplineScore || null,
      });
    }
    return days;
  };
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h1 className="text-4xl sm:text-5xl font-bold bg-gradient-to-r from-blue-400 via-purple-400 to-blue-400 bg-clip-text text-transparent">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
          ...
          {/* Left Column - Task Tracker & Timer */}
            ...
            <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-6">
              ...
              <div className="flex gap-2 mb-6 flex-col sm:flex-row">
                <input
                  ...
                  className="flex-1 bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-white placeholder-gray-400 focus:outline-none focus:border-blue-400 transition"
                />
                <select
                  value={priority}
                  onChange={(e) => setPriority(e.target.value)}
                  className="bg-white/10 border border-white/20 rounded-lg px-3 py-3 text-white focus:outline-none focus:border-blue-400 transition"
                >
                ...
                <button
                  onClick={addTask}
                  className="bg-blue-500 hover:bg-blue-600 rounded-lg px-4 py-3 font-semibold transition transform hover:scale-105 flex items-center gap-2"
                >
                  <Plus size={20} />
                        ...
                        <button
                          ...
                          className={`p-2 rounded-lg transition ${
                            task.status === 'completed'
                              ...
                              : 'bg-white/10 hover:bg-green-500/30 border border-white/20'
                          }`}
                          title="Mark complete"
                        >
                          <Check size={18} />
                        ...
                        <button
                          ...
                          className={`p-2 rounded-lg transition ${
                            task.status === 'failed'
                              ...
                              : 'bg-white/10 hover:bg-red-500/30 border border-white/20'
                          }`}
                          title="Mark failed"
                        >
                          <X size={18} />
              ...
              {tasks.length > 0 && (
                <button
                  onClick={saveDay}
                  className="w-full mt-6 bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 rounded-lg px-4 py-3 font-semibold transition transform hover:scale-105"
                >
              ...
              <div className="flex flex-col items-center gap-6">
                ...
                <div className="flex gap-4">
                  <button
                    ...
                    className="bg-blue-500 hover:bg-blue-600 rounded-lg px-8 py-3 font-semibold transition"
                  >
                  ...
                  <button
                    ...
                    className="bg-white/10 hover:bg-white/20 rounded-lg px-8 py-3 font-semibold transition border border-white/20"
                  >
                    Reset
                  </button>
                </div>
            ...
            <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-6">
              ...
              <ResponsiveContainer width="100%" height={200}>
                <LineChart data={weeklyStats}>
                  ...
                  <Line type="monotone" dataKey="score" stroke="#60a5fa" strokeWidth={3} dot={{ fill: '#60a5fa' }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-7 gap-1">
                    {calendarDays.map((day, idx) => (
                      <div
                        key={idx}
                        className={`aspect-square rounded-lg flex items-center justify-center text-xs font-semibold border border-white/20 ${
                          day ? getDayColor(day.score) : 'bg-transparent border-transparent'
                        }`}
                      >
                        {day?.date}
                      </div>
                  ...
                  <div className="flex gap-2 text-xs mt-4">
                    <div className="flex items-center gap-1">
                      <div className="w-2 h-2 bg-green-500/50 rounded"></div>
                      <span>80%+</span>
                    </div>
                    <div className="flex items-center gap-1">
                    ...
                    <div className="flex items-center gap-1">
                      <div className="w-2 h-2 bg-red-500/50 rounded"></div>
                      <span>Below 60%</span>
                    </div>
                  </div>
                </div>
        ...
        <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div>
              <h3 className="text-lg font-semibold mb-4 text-gray-300">Task Completion</h3>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="text-4xl font-bold text-cyan-400">
                  {dailyHistory.length > 0
                    ? Math.round(
                        (dailyHistory.filter(d => d.completed_percentage >= 70).length / dailyHistory.length) * 100
                      )
                    : 0}
                  %
                </div>
              ...
              <div>
                <p className="text-gray-400 text-sm mb-2">Total Tasks Completed This Week</p>
              ...
              <div>
                ...
                <div className="text-4xl font-bold text-blue-400">
                  {dailyHistory.length > 0
                    ? Math.round(dailyHistory.reduce((sum, d) => sum + d.disciplineScore, 0) / dailyHistory.length)
                    : 0}
                  %
                </div>
              </div>
